defmodule VisionHub.AccountTest do
  ...
  describe "get_users_with_hikvision_devices/0" do
    ...
    test "should return users with Hikvision devices", %{
      ...
      assert Enum.any?(users_with_hikvision, fn u -> u.email == user1.email end)
      assert Enum.any?(users_with_hikvision, fn u -> u.email == user3.email end)

      refute Enum.any?(users_with_hikvision, fn u -> u.email == user2.email end)

      assert Enum.any?(users_with_hikvision, fn u ->
               Enum.any?(u.devices, fn d -> d.brand == "Hikvision" end)
             end)
    end
  ...
  describe "get_users_with_devices/3" do
    ...
    test "should return users with devices matching filters", %{
      user1: user1,
      device1: device1
    } do
      users_with_devices = Account.get_users_with_devices(device1.name, device1.brand, "name")
    ...
    test "should return empty list when no users match filters", _context do
      users_with_devices = Account.get_users_with_devices("Nonexistent", "Hikvision", "name")
    ...
    test "should return users with devices by name filter", %{user1: user1, user3: user3} do
      users_with_devices = Account.get_users_with_devices("Device Job", nil, nil)
    ...
    test "should return users with devices by brand filter", %{
      user1: user1,
      user3: user3,
      device1: device1,
      device2: device2
    } do
      users_with_devices = Account.get_users_with_devices(nil, "Hikvision", nil)
    ...
    test "should return active with devices by ordering" do
      users_with_devices = Account.get_users_with_devices(nil, nil, "name")
    ...
    test "should return all users when no filters are applied", %{
      user1: user1,
      user3: user3,
      device1: device1,
      device2: device2
    } do
      users_with_devices = Account.get_users_with_devices(nil, nil, nil)
